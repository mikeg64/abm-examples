...
def move_toward(target, current, speed):
    ...
    if dist == 0:
        return current
    return (current[0] + speed * dx / dist, current[1] + speed * dy / dist)
...
while not rl.window_should_close():
    ...
    if not game_over:
        ...
        for i in range(NUM_HOUNDS):
            hounds[i] = move_toward(fox_pos, hounds[i], HOUND_SPEED)
    ...
    if game_over:
        ...
        for h in hounds:
            rl.draw_circle(int(h[0]), int(h[1]), HOUND_RADIUS, rl.DARKGREEN)
